...
{
    /// <summary>
    /// Protection adapter for a Button.
    ///
    /// A Button fires on press, so like the Toggle the mouse-down itself is the
    /// action.
    ///
    ...
    /// </summary>
    public class GHShieldButtonAttributes : GH_ButtonObjectAttributes, IGHShieldAttributes
    {
        public GHShieldButtonAttributes(Grasshopper.Kernel.Special.GH_ButtonObject owner)
            : base(owner)
        {
        }
        ...
        private GH_ObjectResponse Block(GH_Canvas sender, GH_CanvasMouseEvent e)
        {
            ...
            Log.Debug($"Frozen a Button '{Owner.NickName}': interaction blocked.");
